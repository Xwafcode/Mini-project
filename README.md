# Mini-project

Repositori ini memuat kumpulan mini proyek yang mencakup berbagai pemodelan kecerdasan buatan, pemrosesan bahasa alami (NLP), dan regresi machine learning.

## Daftar Proyek

1. **Agent Based Model - Simulasi Ketergantungan AI pada Mahasiswa Dalam Penyelesaian Tugas**
   - Berisi pemodelan simulasi berbasis agen (Agent-Based Model) untuk menganalisis dinamika ketergantungan mahasiswa terhadap AI dalam mengerjakan tugas akademik.

2. **NLP - Question Generator**
   - Mini proyek di ranah Natural Language Processing (Pemrosesan Bahasa Alami) untuk generate soal berdasarkan konteks dan jawaban.

3. **Regression_Insurance (HealthCost AI)**
   - Proyek analitik dan machine learning berbasis Random Forest Regression untuk memprediksi biaya asuransi medis. Dilengkapi dengan antarmuka interaktif menggunakan Streamlit.

4. **CBR_Putusan_Pencurian**
   - Sistem Case-Based Reasoning (CBR) untuk analisis putusan pengadilan tindak pidana pencurian (Pasal 362 vs Pasal 363 KUHP) dari Pengadilan Negeri Malang. Menggunakan pendekatan retrieval TF-IDF dan IndoBERT, serta antarmuka web interaktif berbasis Streamlit.
